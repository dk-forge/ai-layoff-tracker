...
<main class="alt-wrap alt-method-page">
  ...
  */ ?>
  ...
  <section class="alt-method-sec" id="m-ai">
    ...
    <p>We sort each mention of AI into five classes: a <em>primary cause</em>, a contributing cause, a selection or operations tool, background context, and an explicit denial. Only a primary or contributing cause can earn the AI tag. Each one must carry an exact supporting quote from the source text. AI investment, projections about future automation, and AI used to select workers do not qualify on their own. <b>The strict AI tag also requires the employer to have attributed the cuts to AI.</b> A news report counts when it quotes or reports the employer naming AI, because the words are still the employer&rsquo;s. A journalist&rsquo;s own characterisation does not count, even when it is unambiguous. Next to the strict tag we keep a second, separately labeled measure: <b>AI-linked, broad</b>. The two tiers differ on both counts, speaker and strength. The strict tier holds cuts the employer itself blamed on AI, with the exact quote on file. The broad tier holds every other AI link, so a press characterisation the employer never made belongs there. It also counts cuts made while a company funds an AI pivot, and AI-driven market disruption. The API reports the strict tag in <code>ai_explicit</code> and the broad measure in <code>ai_broad_jobs</code>. We never mix the broad measure into the strict verified-AI totals, so the two are never added together. We publish every strict AI attribution with its quote on the <a href="<?php echo esc_url(home_url('/ai-layoff-tracker/ai-quotes/')); ?>"><?php echo esc_html(alt_page_link_label('page-ai-quotes.php', 'AI layoffs, in the employer\'s own words')); ?></a> page.</p>
  </section>

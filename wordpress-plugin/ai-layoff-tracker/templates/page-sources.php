...
$alt_gap_states = array(
    ...
    array('Nevada', 'DETR publishes a cumulative WARN master PDF, but the site now sits behind a bot-wall (Akamai) that blocks automated access from data-center servers, so our importer cannot reach the file from its cloud host. The PDF loads fine in a normal browser, and Nevada cuts still reach the tracker through SEC filings and named news. We will re-enable the direct import if DETR removes the block or offers a data feed.', 'Publishes, but bot-walled to automated access', 'https://detr.nv.gov/Page/WARN', 'NV'),

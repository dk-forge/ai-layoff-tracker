...
$alt_gap_states = array(
    ...
    array('Oklahoma', 'The Employment Security Commission publishes WARN notices only through a JavaScript portal that renders no list a reader or an importer can parse. A routine coverage review in August 2026 checked eight routes and found no machine-readable surface anywhere. The portal page carries no table markup, offers no spreadsheet, CSV or PDF download, and exposes no documented feed or API. The state open-data catalogue holds no WARN dataset, and the state\'s own workforce page links to the portal instead of publishing a list. Crawling is expressly permitted, so this is a technical absence and not a refusal. The notices themselves do carry affected-employee counts, so the data would be usable the day it is published in a readable form. We estimate 30 to 60 notices a year are missed this way. Oklahoma cuts still reach the tracker through SEC filings and named news.', 'Portal only, no readable feed', 'https://www.employoklahoma.gov/Participants/s/warnnotices', 'OK'),
  ...
  <div class="alt-health-table-wrap"><table class="alt-sortable alt-sources-table">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>Global news in 65+ languages, searched<?php echo esc_html($alt_cad_sp); ?>. We read GDELT's own published index files directly, both the English feed and the translated feed. Discovery therefore does not depend on its query service being up. Each run records the newest index file it consumed. The query mirror and the query service are fallbacks. Allowlist of trusted outlets only, never open-web crawling. Before any article is read, the publisher's robots.txt is checked under our own named agent (AiLayoffTracker, with a contact address). A refusal is honoured and counted, never worked around, and a robots.txt we cannot read is treated as a refusal.</td>

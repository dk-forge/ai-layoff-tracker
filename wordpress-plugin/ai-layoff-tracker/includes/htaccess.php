...
function alt_htaccess_block_lines() {
    ...
    return array(
        ...
        'Header unset Pragma',
        'Header unset Expires',
        '# Short, and deliberately WITHOUT stale-while-revalidate: two independent',
        '# shared caches sit above this origin (Cloudflare over a Railway proxy) and',
        '# their staleness windows add, so swr=600 let readers see a superseded build',
        '# for 18 minutes after a deploy (measured 2026-08-05). Nothing here can purge',
        '# them, so the lifetime IS the guarantee. stale-if-error keeps the outage',
        '# protection without the deploy staleness. Must stay in step with',
        '# alt_public_page_cache_headers() in includes/shortcodes.php.',
        'Header set Cache-Control "public, max-age=60, s-maxage=60, stale-if-error=600"',
        '</If>',

...
function alt_db_upsert(array $row, $allow_suppressed = false) {
    global $wpdb;
    $table = alt_db_table();
    ...
    if (!$allow_suppressed && alt_is_suppressed($data['dedup_hash'])) {
        return 0;
    }
...
function alt_api_merge_events(WP_REST_Request $r) {
    ...
    $reports = alt_source_reports_table();
    $reason = trim((string) $r->get_param('reason'));
    $out = array('merged_rows' => array(), 'merged_records' => array(),
        'net_jobs_removed' => 0, 'not_found' => array(), 'rejected' => array());
    foreach ((array) $r->get_param('merges') as $merge) {
        ...
        foreach (array_unique(array_map('intval', (array) ($merge['duplicate_ids'] ?? array()))) as $duplicate_id) {
            ...
            $fields = array('id', 'company', 'job_count', 'layoff_date', 'country',
                'source_type', 'source_name', 'source_url', 'dedup_hash');
            $keeper_record = $duplicate_record = array();
            foreach ($fields as $field) {
                $keeper_record[$field] = $keeper[$field] ?? null;
                $duplicate_record[$field] = $duplicate[$field] ?? null;
            }
...
/**
 * Restore source-proven rows that /merge-events removed in error.
 *
 * This is intentionally not a generic undelete. The submitted hash must still
 * be on the suppression list with a `merged:` reason, the hash must not already
 * exist, and the row is inserted while suppression remains armed. Only after a
...
function alt_api_restore_merged_rows_legacy_unused(WP_REST_Request $r) {
    ...
    foreach ($entries as $e) {
        ...
        // A backup may prove that the removed row was editor-pinned. Preserve
        // that governance state; otherwise the next bulk import could silently
        // overwrite a correction merely because the row received a new id.
        $restore_state = array();

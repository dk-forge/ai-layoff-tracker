...
under /blog/, and the sibling tracker's jobs failed one after another. Then the
CI failure alert fired for them and POSTed to `/alert` — a route ON THE HOST
THAT WAS DOWN. "HTTP 504 from /alert", four times. This repo's `ci_alert.py` is
the same design against the same host, so it had the same defect; it simply had
no red run to report that night.

**The alerting system depended on the host it was alerting about.** The one
moment the alarm is most needed is exactly the moment it cannot ring — the same
failure class this project keeps finding (a monitor silent precisely when it
matters), one layer up. That night's outage was found by the owner in a browser,
not by anything here.

WHY A COMMITTED FILE AND NOT A LONGER RETRY
-------------------------------------------
Retrying inside the run is necessary and not sufficient. That outage lasted
...
goes into a file in the repository, which survives the runner, the outage and
the day, and `alert-drain.yml` delivers it afterwards. A runner's /tmp does not
outlive the job, and the alert has to.
...
* Delivered entries become bounded history (HISTORY_KEPT) rather than being
  forgotten, so an outage can be reconstructed after the fact.
...
#: drain every 30 minutes, 12 attempts is roughly six hours of continuous

...
DEFAULT_REPO = "dk-forge/ai-layoff-tracker"
BRANCH = "main"
GRACE = timedelta(hours=2)
ISSUE_TITLE = "Main is not green"
MARKER = "<!-- main-green-check:running-issue -->"
SET_PREFIX = "<!-- main-green-check:set="
# Import the train's deploy reconcile and PRINT what it would do. Never judged.
PRINT_DEPLOY_DRIFT = True
...
WORKFLOWS: tuple[Workflow, ...] = (
    Workflow("tests.yml", "the test suite, every push and every train merge"),
    Workflow("style-standard.yml", "required gate, on push and daily"),
    Workflow("card-contract.yml", "required gate, on push and daily"),
    Workflow("version-collision.yml", "required gate; it has no schedule and no "
             "dispatch, so a train merge cannot start it and age says nothing",
             max_age_days=None),
    Workflow("deploy-plugin.yml", "what readers actually get; path-filtered, "
             "so a quiet week is normal and only its last result is judged",
             max_age_days=None),
)
...
# --------------------------------------------------------------------------
def print_deploy_drift(repo: str) -> None:
    """PRINTED, NOT JUDGED. The logic is the train's own; this imports it."""
    print("\nplugin tree vs last green deploy (printed, not judged):")
    try:
        import dataclasses
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        import merge_train as mt
        root = Path(__file__).resolve().parent.parent
        cfg = dataclasses.replace(mt.Config.load(root / ".github" / "merge-train.json"),
                                  repo=repo, post_merge_workflows=())
        rep = mt.Report()
        problems = mt.sync_main(mt.GitHubClient(repo, root), cfg, rep, dry_run=True)
        if not rep.lines and not problems:
            print("  main's plugin tree matches the last successfully deployed SHA")
    except Exception as exc:  # noqa: BLE001 - a print must never change the verdict
        print(f"  UNKNOWN, could not be compared: {exc}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if PRINT_DEPLOY_DRIFT:
        print_deploy_drift(repo)

...
runners hold `WP_API_KEY` and `OPENROUTER_API_KEY`, they run unattended twice a
...
anything the lock did not vouch for, transitively. `requirements-min.lock` is
the same thing for the light install (openai + requests) so a health job does
not pay for pdfplumber and google-cloud-bigquery, which is exactly why those
workflows were doing a bare install in the first place.
...
LOCKS = ("requirements.lock", "requirements-min.lock")
...
class EveryLockIsFullyHashed(unittest.TestCase):
    ...
    def test_the_two_locks_never_disagree_about_a_shared_package(self):
        """The light install must be a SUBSET of the full one, not a variant.
        ...
        full, minimal = pins("requirements.lock"), pins("requirements-min.lock")
...
class TheInputsStayHumanEdited(unittest.TestCase):
    def test_every_package_named_in_an_input_is_present_in_its_lock(self):
        ...
        pairs = [("requirements.txt", "requirements.lock"),
                 ("requirements-min.txt", "requirements-min.lock")]

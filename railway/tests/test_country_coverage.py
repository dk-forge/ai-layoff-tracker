...
class EveryEntryIsCheckable(unittest.TestCase):
    ...
    def test_cambodia_is_a_classification_not_permanent_backlog(self):
        """Article 95 and the ministry catalogue settle the declared state."""
        entry = cc.REGISTER["Cambodia"]
        self.assertEqual(entry["class"], cc.REGIME_NO_AGGREGATE)
    ...
    def test_south_korea_is_a_classification_not_permanent_backlog(self):
        """Primary statutes and the ministry catalogue settle the declared state."""
        entry = cc.REGISTER["South Korea"]
        self.assertEqual(entry["class"], cc.REGIME_NO_AGGREGATE)
        self.assertIn("law.go.kr", entry["cite"])
        self.assertNotIn("South Korea", cc.ACKNOWLEDGED_BACKLOG)

    def test_thailand_is_a_classification_not_permanent_backlog(self):
        """The official consolidated Act settles Thailand's narrow regime."""
        entry = cc.REGISTER["Thailand"]
        self.assertEqual(entry["class"], cc.REGIME_NO_AGGREGATE)
        self.assertIn("mol.go.th", entry["cite"])
        self.assertNotIn("Thailand", cc.ACKNOWLEDGED_BACKLOG)

    def test_new_zealand_is_a_classification_not_permanent_backlog(self):
        """Current official law and guidance settle the no-regime finding."""
        entry = cc.REGISTER["New Zealand"]
    ...
    def test_indonesia_is_a_classification_not_permanent_backlog(self):
        """PP 35/2021 article 38 settles the government-reporting duty."""
        entry = cc.REGISTER["Indonesia"]
        self.assertEqual(entry["class"], cc.REGIME_NO_AGGREGATE)
        self.assertIn("jdih.kemnaker.go.id", entry["cite"])
        self.assertNotIn("Indonesia", cc.ACKNOWLEDGED_BACKLOG)

    def test_costa_rica_is_a_classification_not_permanent_backlog(self):
        """The current official Labour Code settles the no-regime finding."""
        entry = cc.REGISTER["Costa Rica"]
        self.assertEqual(entry["class"], cc.NO_REGIME)
        self.assertIn("mtss.go.cr", entry["cite"])
        self.assertNotIn("Costa Rica", cc.ACKNOWLEDGED_BACKLOG)

    def test_vietnam_is_a_classification_not_permanent_backlog(self):
        """The official current Code settles the provincial notification duty."""
        entry = cc.REGISTER["Vietnam"]
        self.assertEqual(entry["class"], cc.REGIME_NO_AGGREGATE)
    ...
    def test_hungary_is_a_classification_not_permanent_backlog(self):
        """Current official law settles Hungary's notification regime."""
        entry = cc.REGISTER["Hungary"]
        self.assertEqual(entry["class"], cc.REGIME_NO_AGGREGATE)
        self.assertIn("njt.hu", entry["cite"])
        self.assertNotIn("Hungary", cc.ACKNOWLEDGED_BACKLOG)

    def test_czechia_is_a_classification_not_permanent_backlog(self):
        """Official law and regional reports settle Czechia's published scope."""
        entry = cc.REGISTER["Czechia"]
        self.assertEqual(entry["class"], cc.REGIME_NO_AGGREGATE)

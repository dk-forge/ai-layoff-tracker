...
| Japan — EDINET | The Financial Services Agency provides a documented EDINET API, including a dated document-list endpoint. | **Discovery probe live; promotion package pending review (2026-07-18); connector not live.** `cron.py` lists the newest complete JST day's official metadata twice daily and reports `edinet_jp` source health — nothing is scheduled for download, extraction, or event creation. The evidence-only document stage (`type=1` endpoint), Japanese fixtures and tests now exist; what remains for a reviewer is listed in [PROMOTION_REVIEW.md](PROMOTION_REVIEW.md). A persisted replay cursor is still missing. |
| South Korea — OpenDART | The Financial Supervisory Service provides documented JSON disclosure-list endpoints. | **Discovery probe live; promotion package pending review (2026-07-18); connector not live.** `cron.py` pages the newest complete KST day's disclosure metadata twice daily and reports `opendart_kr` source health under the same no-ingestion rule. The evidence-only document stage (`document.xml` endpoint), Korean fixtures and tests now exist; what remains for a reviewer is listed in [PROMOTION_REVIEW.md](PROMOTION_REVIEW.md). A persisted replay cursor is still missing. |

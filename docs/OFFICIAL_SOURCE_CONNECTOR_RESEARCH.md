...
| Japan — EDINET | The Financial Services Agency provides a documented EDINET API, including a dated document-list endpoint. | **Discovery-only client foundation present; connector not live.** The inactive client uses only the official daily metadata API and retains viewer URLs/status metadata, but has no schedule, document download, extraction, event write, source-health reporting or coverage claim. Activation still needs Japanese-language fixtures, a persisted replay cursor, source-health reporting and an evidence-only document stage. |
| South Korea — OpenDART | The Financial Supervisory Service provides documented JSON disclosure-list endpoints. | **Discovery-only client foundation present; connector not live.** The inactive client paginates official disclosure metadata with documented status/rate handling and retains public viewer URLs, but has no schedule, document download, extraction, event write, source-health reporting or coverage claim. Activation still needs Korean-language body/evidence fixtures, a persisted replay cursor, source-health reporting and an evidence-only document stage. |

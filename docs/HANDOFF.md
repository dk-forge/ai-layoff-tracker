...
- **RELEASED - Claude (agent), 2026-09-22 early, ops green sweep (PR #404,
  ...
  relayed WARN run (35668731673) upserted 45,121 rows against 34,552 and no
  state read dark. Hawaii crawl rotates its start; three health posters retry.
  No plugin file. **Left for a human:** `warn:MS` DARK is quiet by evidence
  (register's newest PDF is Apr-Jun 2026, our frontier; 22.0/yr, p90 gap 45d,
  84d silent); `dedupe_llm` and the digest wait on the owner's OpenRouter key;
  `headline_movement` closes with the commands below.
...
- **RELEASED - Claude, 2026-09-21, branch `fix/train-dispatches-deploy` (PR #394,
  ...
  never deployed (#393). `merge_train.sync_main()` now reconciles main after
  every run by dispatch: `deploy-plugin.yml` when main's plugin tree differs
  from the last green deploy and no deploy ran inside 60 min, `tests.yml` when
  main's head has no run. No plugin file touched, no version reserved.
...
- **HELD by Codex (2026-09-13) — fail before paid work when the host is down.**
  ...
  **Railway repair production-closed (2026-09-13 01:49 CEST).** PR #336 merged
  as `63a0cd7d` after all seven required checks passed, including the live-data
  shard evaluating its invariants. Railway deployment
  `b237d665-befe-4bdd-a7fa-8fe1106aa9d8` reached SUCCESS. Its applied service
  manifest identifies `/railway/railway.toml`, schedules only `0 22 * * *`,
  preserves the code-only watch patterns, and reports numeric
...
- **VERSION CLAIMED (2026-08-29): 2.20.152 — a declared collector that was
  ...
  reported `earnings_ingest` NEVER REPORTED at every session start since the
  inventory shipped; there was no module, no workflow and no `cron.py` line,
  because the transcript ingest was dropped for HTTP 402 in the SAME batch that
  added the label (2.19.84). Rebuilding it is not available on any key we hold:
  FMP transcripts are Ultimate-tier and Finnhub's are Premium, and every
  alternative is a new signup. Plugin files touched: `assets/health.js` (one
  `meta{}` line removed) and `ai-layoff-tracker.php` (version only). Rest is
  `railway/source_inventory.py` (`unimplemented_collectors`),
...
- **NO VERSION CONSUMED (2026-08-19): the Kansas WARN collector, and a fourth
  ...
  was consumed** - the change is `railway/sources/warn_new_states.py`,
  `railway/tests/test_ks_warn_ordering.py` and TECHLOG, so
  `reader_freshness.py` has nothing to verify. Kansas turned out NOT to be
  broken (the state's register genuinely has no notice after 2026-05-01); the
  shipped fix is the latent ordering defect behind it. See TECHLOG 2026-08-19.
  **The collision warning above is now confirmed a fourth time, same night,
  same shape.** A `git add` of three explicit paths still swept in
  `docs/HANDOFF.md` and `docs/RECOVERY.md`, because a concurrent session had
  staged them into the shared index between the edit and the commit; a second
  commit then picked up that session's TECHLOG entry the same way. Both were
  reverted out (`git reset --soft` + `git restore --staged`, then the blob
  rebuilt from `origin/main` via `hash-object`/`update-index` so the shared
  worktree was never touched) and the other session's work is intact. **Staging
  by explicit path is NOT sufficient protection - the index is shared, so a
  path you name can still carry someone else's staged content.** Verify with
  `git show --stat HEAD` AFTER committing, and do the work in an isolated
  worktree from the start, which is where the rest of this session ran.

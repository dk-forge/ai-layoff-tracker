...
| `served its 'Unavailable' placeholder` (`NiNotPublished`) | NISRA's redundancies page lists a month whose datavis report is not uploaded yet: datavis.nisra.gov.uk answers a missing page with HTTP 200 and a hosting placeholder, not a 404 | Nothing to fix. The report is discovered from NISRA's own list, never templated from today's month (2026-09-04), so this can only appear in the hours between NISRA listing a report and uploading it. The placeholder is never cached, so the next run reads the report. If it persists a week, open the `source_url` by hand |
...
- **The cron minutes are :07 and :37, not :00 and :30, and that is deliberate.**
  GitHub queues every repository's on-the-hour schedule at once and documents
  that those are the most delayed; measured 2026-09-06 to 2026-09-11 the
  `0 10 * * *` daily tick started at 13:56 UTC every day and the Monday
  `30 11 * * 1` tick at 16:36. Only the minute moved (2026-09-11); the hours,
  the DST pairing and the slot guard are unchanged, and a run that still starts
  more than an hour late prints `LATE TICK: scheduled HH:MM UTC, started HH:MM
  UTC` in its log. Do not move them back to the hour for tidiness.

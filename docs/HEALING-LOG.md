...
<!-- Entries are appended above this line by `railway/self_heal.py record`,
     which runs from self-heal.yml AFTER a successful auto-merge. Recording
     is best-effort by design: it can never fail a heal, so an empty stretch
     here is not proof no heal happened — cross-check `git log --grep
     'self-heal: auto-merged'` on main. -->
